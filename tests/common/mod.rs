...
pub fn seed_provider_quota(d: &TestDaemon, alias: &str, observed_epoch: i64) {
...
pub fn drop_shutdown_marker(state: &Path) {
...
pub fn stopped_mid_turn_devin() -> (PathBuf, MockDevin, String, i32) {
...
pub fn read_marker(state: &Path) -> Value {
...
pub fn adopt_refusal(d: &TestDaemon, alias: &str) -> Option<String> {
    d.rpc("agent_events", json!({"alias": alias})).unwrap()["events"]
        .as_array()
        .unwrap()
        .iter()
...
pub fn d_pane(mock: &MockDevin, state: &Path, alias: &str, ext: &str) -> PathBuf {
    mock.dir
        .join("tmux-state")
...
pub fn park_idle_labelled_turns(d: &TestDaemon, mock: &MockDevin) -> (String, String, i32) {
    d.register_devin("dv1", None);
    d.register("pm");
    d.wait_agent("dv1", "idle", 20);
    d.rpc("agent_ready", json!({"alias": "dv1"})).unwrap();
...
pub fn message_ids(d: &TestDaemon, alias: &str, id: &str) -> usize {
...
pub fn restart_idle_pty_after_forced_detach(via_signal: bool) {
...
pub fn claude_sessions(dir: &Path) -> Vec<(u32, String)> {
    let mut out = Vec::new();
...
pub fn install_mock_cursor_tui(dir: &Path) -> MockCursorTui {
...
pub fn forced_removals(d: &TestDaemon) -> Vec<Value> {
...
pub fn argv_origin(d: &TestDaemon, mock: &MockClaude, alias: &str) -> String {
...
pub fn cadence_cli(state: &Path, args: &[&str], envs: &[(String, String)]) -> (bool, Value) {
...
pub fn recover(d: &TestDaemon, alias: &str, id: &str) -> cadence_agent::Result<Value> {
    d.operator_rpc(
...
/// `cadence ui stop`.
pub struct OperatorBoard(PathBuf);
...
pub fn start_operator_board(pm: &Path, state: &Path) -> (u16, OperatorBoard) {

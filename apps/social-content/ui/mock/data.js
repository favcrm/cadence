/* module: ui/mock/data.js — the in-memory database for the prototype.
 ...
 * reader is ui/mock/api.js — views never import this directly.
 */
(function (SC) {
  "use strict";
  ...
  const NOW = new Date("2026-09-25T18:40:00+08:00");
  const H = 3600e3, D = 24 * H;
  const at = (h) => new Date(NOW.getTime() + h * H).toISOString(); // h = hours from now
  const ago = (h) => at(-h);
  ...
  SC.mock = {
    NOW, clients, sourcePosts, posts, runs, suggestions, digests, automations, workflows,
    selection: new Set(),   // Library multi-select lives here (UI state of the mock)
    activity: [

...
- The pre-serde duplicate-key scan rejects JSON nested deeper than 128. A body under the 20 KiB cap can otherwise nest thousands of arrays and overflow the daemon stack before serde's recursion limit applies. Array (9,000) and object (2,000) fixtures are rejected, the parser still accepts a following valid document, and a rejected store write leaves revision 0 with no `model_defaults_updated` event before a later valid save commits.

...
- Status: **proposed**. Not accepted. No code may be written against it until
  the operator accepts it (CAD-313 acceptance item 1, risk class `human`,
  trigger 1: auth or identity, `request_actor`, tailnet).
- Date: 2026-09-23
...
- Supersedes, if accepted: the `src/ui.rs` module rule "No auth, by
...
```bash
# The ADR exists, is proposed, and is indexed
test -f docs/adr/0004-operator-identity-web-ui.md
grep -q 'Status: \*\*proposed\*\*' docs/adr/0004-operator-identity-web-ui.md

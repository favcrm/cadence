...
The historical coordinator-only candidate added a separate v8-to-v9
`auto_dispatch_enabled` migration; its rehearsal is retained below as
historical evidence. The combined quota/coordination candidate described
later supersedes that version marker and needs its own isolated rehearsal and
reviewed build; neither prior rehearsal authorizes a live schema change.
...
## Quota integration ordering

The provider-owned quota producer and the coordinator consumer share the
store's schema version marker. Their safe landing order is explicit: PR103
lands first as schema v9 (`agents.quota`), then PR100 rebases and lands its
background-consent column as schema v10 (`monitors.auto_dispatch_enabled`).
The v10 migration also repairs a schema-9 store made by an older PR100
candidate that has the monitor column but not `agents.quota`, so an upgrade
cannot silently skip provider evidence. Automatic admission reads only the
provider-owned quota record, requires the canonical RFC3339 timestamp,
current provider/assignee/thread/account identity, and fails closed for
missing, malformed, stale, or non-available evidence. Caller-editable
`params.quota` is not admission evidence.

This integration branch is a review candidate only. The earlier v6-to-v9
rehearsal above does not prove the v10 migration, quota producer, or live
activation; those require the exact combined head and a separate reviewed
rehearsal.
...
3. Verify build commit, schema 8 for the baseline monitor rollout (or schema
   10 only after the combined quota/coordinator candidate is reviewed), monitor

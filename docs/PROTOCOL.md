...
| `answer_route` | `issue, report` | `{sent, to, message?, duplicate?, state?}`, or `{sent:false, to, undeliverable}` when the asker is not registered (recorded as a daemon-stream `answer_undeliverable` event; the answer stands). CAD-447: queues ONE message (source `answer`, id `answer-<hash of the answer's path>`) to the author of the question the `answer` report `report` names, with the answer text and the report's path. Sent by `cadence report file --kind answer` and the board's answer route right after filing |

...
| `agent_list` | `states?, providers?, kinds?` (each a string or array — any-of; unknown values rejected) | `{agents:[Agent+tasks+capabilities]}` — `tasks` names the alias's non-terminal task assignments; `capabilities` is the registry descriptor |
...
| `job_list` | `state?, states? (any-of, implies all), all?` | `{jobs:[Job+task counts]}` — non-terminal states by default; `state`/`states` select from `draft open done failed cancelled` (unknown rejected) and imply `all` |
...
| `delivery_list` | `issue?, issues?, states?, projects?, open?` | `{records:[Record]}` — the worker loop's records (CAD-431), oldest dispatch first. `issue`/`issues` any-of ticket ids, `states` any-of loop states (`working reviewing unstaffed passed enqueued escalated merged declined closed`; unknown rejected), `projects` any-of tracker keys, `open` keeps only non-terminal rows. A read, open to anyone (the master included) |
...
| `platform_enroll` | `platform, account, scopes, shape?("token"\|"consent"), token?, class?, accept_same_uid_risk?` | `{state:"enrolled", account:{platform, account, scopes, fingerprint, custody, exchange, enrolled_at, by}}` — CAD-366 (ADR 0006 §5.3): enrolls a platform credential into custody. `shape:"token"` takes the operator-minted scoped token, which crosses the socket exactly once and is never returned — the record keeps a `fingerprint` (the `secret` SHA-256 prefix), never the bytes. `shape:"consent"` runs the platform's consent-exchange adapter hook (device code/OTP/scope consent); none is registered yet (CAD-501), so it refuses naming the seam — a `token` field is refused on it. `class` other than `"scoped"` is a personal approval/publish credential and is refused, as are known personal token shapes (`github_pat_`/`ghp_`/`ghu_`/`gho_`/`ghr_`/`glpat-`; leading/trailing whitespace is trimmed before screening). Custody keeps bytes from other uids and the confined master only — same-uid unconfined agents can still read them (ADR 0006 P4 residual) — so while the daemon reports no custody isolation mode a first enroll refuses `custody_unprotected` before any exchange runs; `accept_same_uid_risk` is the operator's explicit override, recorded on `platform_connected` as `custody_risk_accepted:"same-uid"`. Operator only (`operator_connection`); identity-shaped fields refused. CLI `cadence platform enroll` (`--token-stdin` keeps the token off argv) |
| `platform_rotate` | `platform, account, shape?, token?, scopes?, class?` | same shape as enroll — replaces the credential bytes under the same handle; `scopes` omitted keeps the recorded set. Grants and project defaults do not change; the old credential's `credential_revoked` event records `reason:"rotated"`. Operator only |
| `platform_revoke` | `platform, account, reason?` | `{state:"revoked", platform, account, fingerprint, grants_revoked, effects_closed}` — drops the custody bytes, the record, every grant on the account and any project default naming it; pending effects bound to the credential close unanswered with the named reason (§5.3; the `kind:"effect"` requests CAD-506 opens). `reason` reaches audit and agent-visible events, so it is screened against the enrolled credential first — a reason carrying the bytes is refused, never published. Operator only |
| `platform_accounts` | — | `{accounts:[{platform, account, scopes, fingerprint, custody, exchange, enrolled_at, by}]}` — the §5.3 record, handles only; open read |
| `platform_grant` | `agent, platform, account, scopes` | `{state:"granted", grant}` — grants `agent` the scopes on the account; an existing grant widens by union. The account must be enrolled (a grant on nothing is a latent privilege). Operator only |
| `platform_ungrant` | `agent, platform, account, scopes?` | `{state:"revoked", grant}` — `scopes` omitted drops the whole grant. Operator only |
| `platform_grants` | `agent?` | `{grants:[{agent, platform, account, scopes, granted_at, by}]}` — an agent caller reads its own grants only (an `agent` param naming another is refused); the operator reads all or one agent's |
| `platform_check` | `platform, scope, account?, project?, agent?` | `{granted:true, grant}` — the §5.3 pre-flight the proxy gate (CAD-506) runs: refuses naming the missing scope before any platform traffic, and touches no custody. An agent checks its own grants only; the operator names `agent`. `account` omitted resolves the project's default (`project`, else the calling agent's own project) |
| `platform_defaults` | — | `{defaults:[{project, platform, account, set_at, by}]}` — open read |
| `platform_default_set` | `project, platform, account` | `{state:"set", default:{project, platform, account, by}}` — `project` must be a real `<pm>/<key>`; the account must be enrolled. Operator only |

`alias`, `provider`, `message` ids: `^[a-z0-9][a-z0-9-]{0,63}$`.
`text`: 1–48000 chars. `reply_to` may not equal `alias`.

**List filters (CAD-437).** Every `ls`/`list` command speaks one
grammar, and daemon-owned data is filtered server-side by the list
RPCs (`agent_list`, `job_list`, `delivery_list` take the same value
sets as their CLI flags):

- A value flag repeats and comma-joins — `--status doing --status
  review` ≡ `--status doing,review` — and matches ANY of its values.
  The RPC twin accepts a string or an array.
- Different flags AND — a row must satisfy every flag given.
- An unknown value is rejected naming the valid set (never an empty
  page).
- `--json` prints the rows as JSON (RPC-backed lists are JSON
  already); `--limit N` caps the count; `--sort KEY` orders rows
  (`-KEY` descending, a per-command documented key set, ties broken on
  the row's identity key); `--fields a,b` keeps only those keys and is
  `--json`-only.
- `--since`/`--until` take `30m`/`24h`/`7d` lookbacks, an ISO
...
| `answer_route` (CAD-447) | the answer's own recorded author, by the connection: the proven operator for an `operator` answer, the agent itself (the master included) for its own | any other caller — an agent routing an answer filed as `operator` or as another agent, a detached child of an agent, a request carrying an identity-shaped field; an `answer` that is not a valid report naming a question on the same ticket. Only one answer per question is ever sent (the id is the question's); a later one routes nothing (`why: already answered`), so no answerer — the master included — can reach a past asker at will. A retry or concurrent second call queues nothing new |
...
| `agent_recover_submit` (CAD-152) | the proven operator, or the target's own PM (`authorize_agent_mutation`, `peer::may_mutate_agent` controlled class) | the target itself, a peer, another group's PM, a detached child of an agent; `by`/`pane`/… in the request |
...
| `operator_link_mint`, `operator_sessions`, `operator_secret_rotate` (CAD-313) | the proven operator (`operator_proof`) that also presents the operator secret (`<state>/operator/secret`, strict modes) — `cadence ui login`, `ui sessions`, `ui login --rotate` | every agent, a detached child without the secret, a wrong or absent secret, a secret file with a loose mode |
| `operator_session_open` (CAD-313) | whoever holds the login nonce (single use, 120 s, one origin) — the board, for `POST /api/session` — on a connection that derives no agent | a spent, expired, unknown or wrong-origin nonce (`login_link`); a connection that derives an agent (`session_from_agent`), which spends the nonce |
| `operator_session_check`, `operator_session_logout`, `operator_session_stolen` (CAD-313) | whoever holds the session token in the request; no answer ever carries a stored credential | an unknown token (no session) |
| `platform_enroll`, `platform_rotate`, `platform_revoke`, `platform_grant`, `platform_ungrant`, `platform_default_set` (CAD-366) | the proven operator (`operator_connection`) — custody authority is connection-bound; `agent` on grant verbs names the TARGET, never the caller | every agent, and a detached child of any agent |
| `platform_grants`, `platform_check` (CAD-366) | an agent for its own grants only (`agent` naming another is refused); the proven operator reads all or checks for a named holder | an agent asking about another agent; a detached child of an agent (derives no identity and is not provably the operator) |
...
## Master wakes (CAD-445)

Work continues without the operator chatting "go ahead": the daemon
queues one system message (`source: "wake"`, text `[wake] …`) to the
master when
...
refuses a message id starting `sys-` and a daemon source (`wake`, `answer`) on

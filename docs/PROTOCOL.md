...
| `shutdown` | — | `{state:"stopping"}`; daemon stops actors (bounded), writes the clean-stop marker, then exits. Caller rule (CAD-384): the proven operator, or the agent holding the live rollout lease (the rollout owner's `daemon restart` from its own pane); in a sandbox, also a caller tied to none of its agents. Every other agent, and a detached child of one, is refused |
...
| `shutdown` (CAD-384) | the proven operator; the agent holding the live rollout lease; in a sandbox, a caller tied to none of its agents | every other agent, and a detached child of one |

...
| `shutdown` (CAD-384) | the proven operator; the agent holding the live rollout lease under a live operator grant; in a sandbox, a caller tied to none of its agents | every other agent (an ungranted or revoked holder included), and a detached child of one |

...
| `operator_session_open`, `operator_session_check`, `operator_session_logout`, `operator_session_stolen` (CAD-313) | whoever holds the credential in the request: a login nonce (single use, 120 s, one origin) or a session token. The board calls them; no answer ever carries a stored credential | a spent, expired, unknown or wrong-origin nonce (`login_link`) |

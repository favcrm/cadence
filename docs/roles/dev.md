...
3. Tests first where you can (`tdd` skill). Every new behaviour gets a test; flaky timing uses deadline polls on the condition itself (or on the event that proves the daemon looked, for "nothing happens" checks), never fixed sleeps. A sleep stays only where elapsed time is the behaviour under test — a stall or staleness timer, or an absence window with nothing observable to poll — with a comment saying which.
4. Before you report: rebase onto `origin/main` (keep main's code in conflicts), prove no net deletions outside your lane with `rtk proxy git diff origin/main...HEAD --stat`, then run the DEVELOPER gates in the foreground: `cargo fmt --all -- --check`, `cargo clippy --all-targets --all-features -- -D warnings`, `cargo test --lib --bins --test board`, your new tests 5× in isolation, and the integration groups your change touches (`cargo test --test <area> <filter>`, one filter per call — the areas are the `tests/*.rs` binaries: `daemon`, `agents_pty`, `pty_lifecycle`, `pty_interact`, `managed_claude_codex`, `threads`, `tracker_issue`, `plan_work`, `dispatch_jobs_monitor`, `audit_review`, `build_slot`, `delivery`, `reports_memory`, `board_ui`, `backup_rollout`). Choose the binaries by grepping the callers of what you changed — the RPC method, CLI verb, helper or event name (`rg -n '<name>' tests/`) — not by guessing from the file name. **After DevOps confirms the build-slot service is deployed, wrap every cargo build/test/clippy in a build slot**: `cadence build-slot run build -- cargo clippy --all-targets --all-features -- -D warnings`, `cadence build-slot run test -- cargo test --lib --bins --test board` (source the worktree `.env` first if it exists — it exports `CARGO_BUILD_JOBS` and `CADENCE_BUILD_SLOT`). **Do not run the full integration suite**: CI runs it on every push and the reviewer runs it once per PR (`cadence review`) under the host suite lock. Exceptions — run it once, at the end, when the kickoff says so or your change touches any of these, because a filtered run cannot see its blast radius:
   - `src/daemon.rs`, `src/store.rs`, `src/adapter/` — they sit under every daemon-booting test.
   - `src/main.rs` — the CLI; the suites drive the built binary (`CARGO_BIN_EXE_cadence`, 110+ call sites across the `tests/` area binaries, and most `tests/board.rs` tests through `cli`/`cli_run`), so argument parsing, dispatch or output shape can break groups far from yours.
   - The shared test harness — `TestDaemon` and its `wait_*`/`rpc` helpers, the mock installers (`install_mock_*`, `MOCK_*` scripts), `daemon_opts`/`test_env` and the suite-slot guard in `tests/common/`; `cli*`, `UiDaemon`, `mem_fx` and the other fixtures in `tests/board.rs`. Every group calls them, so a helper change alters them all at once.

   Run it as `scripts/cadence-nextest --all-targets` (the pinned runner, installed once per host by `scripts/install-cadence-nextest`; it takes the host lock itself) or `cargo test --all-targets`, both with `CARGO_BUILD_JOBS=4 CADENCE_SUITE_LOCK=$HOME/.local/state/cadence/suite.lock`.

...
## Host courtesy
This host runs several lanes at once. Cap your builds with `CARGO_BUILD_JOBS=4` (the worktree `.env` sets it), never run two cargo commands at the same time in your own lane — `cadence build-slot run <kind> -- cargo …` serializes them fairly across lanes — and do not start a full suite while another one holds `CADENCE_SUITE_LOCK`.

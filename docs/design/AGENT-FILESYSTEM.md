...
| It reads only its views (CAD-439) | Claude Code auto-allows read-only Bash commands (`id`, `ps`, `echo <glob>`, `cat` in its working dirs) in every mode, `dontAsk` included, and `--file` verbs read any path, so the daemon launches the provider under `cadence confine` (Linux Landlock): system trees, the Claude CLI's own state (`~/.claude`, `~/.claude.json`, its install), the programs it runs, the tracker, and the master's own cwd, temp dir and briefing. Nothing else of `$HOME`, the state dir or `/proc`; inherited by every descendant, `setsid` included. `master start` refuses where Landlock is unavailable (older kernels, non-Linux). The policy is logged to the provider log; `CADENCE_MASTER_CONFINE_READ`/`_WRITE` on the daemon add paths | `~/.claude` (the Claude login) stays readable to the tree: Claude's path check denies `cat` there, an allowlisted `--file` verb does not. Network and unix-socket connects are not confined |

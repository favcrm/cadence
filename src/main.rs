...
fn run_app(state_dir: &Path, action: AppAction) -> Result<i32> {
...
fn run() -> Result<i32> {
    ...
    match cli.command {
        ...
        Commands::Platform { action } => run_platform(&state_dir, &action),
        Commands::Plan { action } => run_plan(&state_dir, action),
        Commands::App { action } => run_app(&state_dir, action),
...
fn run_platform(state_dir: &Path, action: &PlatformAction) -> Result<i32> {
    let rpc = |method: &str, params: Value| client::rpc(state_dir, method, params);
